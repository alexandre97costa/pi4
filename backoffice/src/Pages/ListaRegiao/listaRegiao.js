import React from "react";

export default function ListaRegiao(props){

    return (
        <div className='row'>
             <div className='col-12'>
 
                 <div className='col-md-2 ms-auto'>
 
                     <div className='dropdown'>                                                       
                         <button class="btn  bg-white border rounded-2 shadow-1 dropdown-toggle me-md-3" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                            <span>Filtro 1</span> 
                         </button>
                         <div className='dropdown-menu'>
                             <div className='dropdown-item'>Action</div>
                             <div className='dropdown-item'>Action</div>
                             <div className='dropdown-item'>Action</div>                                
                         </div> 
 
                         <button class="btn  bg-white border rounded-2 shadow-1 dropdown-toggle " type="button" data-bs-toggle="dropdown" aria-expanded="false">
                             Filtro 1
                         </button>
                         <div className='dropdown-menu'>
                             <div className='dropdown-item'>Action</div>
                             <div className='dropdown-item'>Action</div>
                             <div className='dropdown-item'>Action</div>                                
                         </div>                          
                     </div>
                 
                 </div>
                                  
 
                  
                 <div className='card p-3 mb-5 shadow bg-body rouded border-0 mt-2'>
                     <div className='row p-3'>
                         <div className='col-12'>
                             <div className='row'>
                                 
                                 
                                 <div className='row'>
                                     <div className='col-12 pb-4'>
                                         <div className='list-group list-group-flush'>
 
                                         <div className='list-group-item  border-secondary'>
                                                 <div className='row'>
                                                     <div className='col-12 pb-1'>
                                                         <div className='row'>
                                                             <div className='col-5'>
                                                                 <h5># Nome</h5>
                                                             </div> 
                                                             <div className='col-2 '>
                                                                 <h5>Categoria</h5>
                                                             </div>  
                                                             <div className='col-md-3 ms-auto text-center'>
                                                                 <h5>Ações</h5>
                                                             </div>                                                             
                                                         </div>
                                                     </div>
                                                 </div>                                
                                             </div> 
 
 
                                             <div className='list-group-item '>
                                                 <div className='row'>
                                                     <div className='col-12 pb-1'>
                                                         <div className='row'>
                                                             <div className='col-5'>
                                                                 <p>02 José António Gomes</p>
                                                             </div>
                                                             <div class="col-2 ">224 Tondela</div>
                                                             
                                                             <div className='col-md-2 ms-auto'>
                                                                 <button className=' btn btn-outline-danger bi bi-trash-fill me-md-2'></button> 
                                                                 <button className='btn btn-outline-warning bi bi-pencil-fill'></button> 
                                                             </div> 
 
                                                         </div>
                                                     </div>
                                                 </div>                                
                                             </div> 
 
                                             <div className='list-group-item '>
                                                 <div className='row'>
                                                     <div className='col-12 pb-1'>
                                                         <div className='row'>
                                                             <div className='col-5'>
                                                                 <p>11 Luisa Machado Castro</p>
                                                             </div>
                                                             <div class="d-inline p-2  rounded-2 col-2 ">224 Armamar</div>
                                                             
                                                             <div className='col-md-2 ms-auto'>
                                                                 <button className=' btn btn-outline-danger bi bi-trash-fill me-md-2'></button> 
                                                                 <button className='btn btn-outline-warning bi bi-pencil-fill'></button> 
                                                             </div> 
 
                                                         </div>
                                                     </div>
                                                 </div>                                
                                             </div>
 
                                             <div className='list-group-item '>
                                                 <div className='row'>
                                                     <div className='col-12 pb-1'>
                                                         <div className='row'>
                                                             <div className='col-5'>
                                                                 <p>21 Rodrigo Miguel</p>
                                                             </div>
                                                             <div class="d-inline p-2 rounded-2 col-2 ">224 Lamego</div>
                                                             
                                                             <div className='col-md-2 ms-auto'>
                                                                 <button className=' btn btn-outline-danger bi bi-trash-fill me-md-2'></button> 
                                                                 <button className='btn btn-outline-warning bi bi-pencil-fill'></button> 
                                                             </div> 
 
                                                         </div>
                                                     </div>
                                                 </div>                                
                                             </div> 
 
                                             <div className='list-group-item '>
                                                 <div className='row'>
                                                     <div className='col-12 pb-1'>
                                                         <div className='row'>
                                                             <div className='col-5'>
                                                                 <p>31 Amália Rodrigues Silva</p>
                                                             </div>
                                                             <div class="d-inline p-2 rounded-2 col-2 ">224 Cinfães</div>
                                                             
                                                             <div className='col-md-2 ms-auto'>
                                                                 <button className=' btn btn-outline-danger bi bi-trash-fill me-md-2'></button> 
                                                                 <button className='btn btn-outline-warning bi bi-pencil-fill'></button> 
                                                             </div> 
 
                                                         </div>
                                                     </div>
                                                 </div>                                
                                             </div> 
 
                                             <div className='list-group-item '>
                                                 <div className='row'>
                                                     <div className='col-12 pb-1'>
                                                         <div className='row'>
                                                             <div className='col-5'>
                                                                 <p>36 Dinis Martins</p>
                                                             </div>
                                                             <div class="d-inline p-2 rounded-2 col-2 ">224 Mangualde</div>
                                                             
                                                             <div className='col-md-2 ms-auto'>
                                                                 <button className=' btn btn-outline-danger bi bi-trash-fill me-md-2'></button> 
                                                                 <button className='btn btn-outline-warning bi bi-pencil-fill'></button> 
                                                             </div> 
 
                                                         </div>
                                                     </div>
                                                 </div>                                
                                             </div> 
 
                                             <div className='list-group-item '>
                                                 <div className='row'>
                                                     <div className='col-12 pb-1'>
                                                         <div className='row'>
                                                             <div className='col-5'>
                                                                 <p>22 Priscila Lopes</p>
                                                             </div>
                                                             <div class="d-inline p-2   rounded-2 col-2 ">224 Nelas</div>
                                                             
                                                             <div className='col-md-2 ms-auto'>
                                                                 <button className=' btn btn-outline-danger bi bi-trash-fill me-md-2'></button> 
                                                                 <button className='btn btn-outline-warning bi bi-pencil-fill'></button> 
                                                             </div> 
 
                                                         </div>
                                                     </div>
                                                 </div>                                
                                             </div> 
 
                                             <div className='list-group-item '>
                                                 <div className='row'>
                                                     <div className='col-12 pb-1'>
                                                         <div className='row'>
                                                             <div className='col-5'>
                                                                 <p>25 Mateus Almeida</p>
                                                             </div>
                                                             <div class="d-inline p-2  rounded-2 col-2 ">224 Viseu</div>
                                                             
                                                             <div className='col-md-2 ms-auto'>
                                                                 <button className=' btn btn-outline-danger bi bi-trash-fill me-md-2'></button> 
                                                                 <button className='btn btn-outline-warning bi bi-pencil-fill'></button> 
                                                             </div> 
 
                                                         </div>
                                                     </div>
                                                 </div>                                
                                             </div> 
 
                                             <div className='list-group-item '>
                                                 <div className='row'>
                                                     <div className='col-12 pb-1'>
                                                         <div className='row'>
                                                             <div className='col-5'>
                                                                 <p>09 Sara Martins Campos</p>
                                                             </div>
                                                             <div class="d-inline p-2  rounded-2 col-2 ">224 Resende</div>
                                                             
                                                             <div className='col-md-2 ms-auto'>
                                                                 <button className=' btn btn-outline-danger bi bi-trash-fill me-md-2'></button> 
                                                                 <button className='btn btn-outline-warning bi bi-pencil-fill'></button> 
                                                             </div> 
 
                                                         </div>
                                                     </div>
                                                 </div>                                
                                             </div> 
 
                                             <div className='list-group-item '>
                                                 <div className='row'>
                                                     <div className='col-12 pb-1'>
                                                         <div className='row'>
                                                             <div className='col-5'>
                                                                 <p>07 João Pedro Pereira</p>
                                                             </div>
                                                             <div class="d-inline p-2  rounded-2 col-2 ">224 Satão</div>
                                                             
                                                             <div className='col-md-2 ms-auto'>
                                                                 <button className=' btn btn-outline-danger bi bi-trash-fill me-md-2'></button> 
                                                                 <button className='btn btn-outline-warning bi bi-pencil-fill'></button> 
                                                             </div> 
 
                                                         </div>
                                                     </div>
                                                 </div>                                
                                             </div> 
 
                                             <div className='list-group-item '>
                                                 <div className='row'>
                                                     <div className='col-12 pb-1'>
                                                     </div>
                                                 </div>                                
                                             </div>  
                                         </div>
                                     </div>
                                 </div>  
                                 <div className='row'>
                                     <div className='col-12 '>
                                         <div className='col-md-2 ms-auto'>                                       
                                                 <div class="pagination">
                                                     <li class="page-item ">
                                                     <a class="page-link" href="#" aria-label="Previous">
                                                         <span aria-hidden="true">&laquo;</span>
                                                     </a>
                                                     </li>
                                                     <li class="page-item"><a class="page-link" href="#">1</a></li>
                                                     <li class="page-item"><a class="page-link" href="#">2</a></li>
                                                     <li class="page-item"><a class="page-link" href="#">3</a></li>
                                                     <li class="page-item">
                                                     <a class="page-link" href="#" aria-label="Next">
                                                         <span aria-hidden="true">&raquo;</span>
                                                     </a>
                                                     </li>
                                                 </div>
                                             
                                         </div>
                                     </div>
                                 </div>   
                             </div>
                         </div>
                     </div> 
                 </div>
             </div>
        </div>   
    );
   
}